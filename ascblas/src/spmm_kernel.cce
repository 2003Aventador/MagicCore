...
// __aicore__ inline void transpose(__cbuf__ half *matrix, int M, int N) {
//     int total = M * N;
    ...
    for (int64_t vec_row = 0; vec_row < m_vec; vec_row++) {
        ...
        for (int k_step = 0; k_step < k_steps; k_step++) {
            ...
            for(int n_step = 0; n_step < n_steps; n_step++){
                ...
                load_cbuf_to_ca(
                    ...
                    static_cast<uint16_t>(0),                               // dstGap
                    static_cast<uint8_t>(0),                                // sid
                    true,                                                   // transpose, 这里的分形转置只为了适配后续的MMA计算，而且更适合叫重排布
                    inc                                                     // addr_cal_mode
                );
                load_cbuf_to_cb(
                    dst_L0_b,
                    ...
                    static_cast<uint16_t>(0),                           // dstGap
                    static_cast<uint8_t>(0),                            // sid
                    true,                                               // transpose, 这里的分形转置只为了适配后续的MMA计算
                    inc                                                 // addr_cal_mode
                );
        ...
        */
        ...
        copy_matrix_cc_to_gm(
            ...
            static_cast<uint8_t>(0b00000),          // QuantRPE
            static_cast<uint8_t>(0b000),            // ReLUPRE
